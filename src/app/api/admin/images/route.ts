...
import { PRO_UPLOAD_DIR, PRO_UPLOAD_PUBLIC } from "@/lib/admin-files";
...
const ROOTS = [
  { dir: () => PRO_UPLOAD_DIR, publicBase: PRO_UPLOAD_PUBLIC },
  {
    dir: () => path.join(process.cwd(), "public", "documents"),
    publicBase: "/documents",
  },
  {
...
export async function POST(request: Request) {
  ...
  const maxBytes = ext === ".pdf" ? MAX_PDF_BYTES : MAX_UPLOAD_BYTES;
  ...
  const isPdf = ext === ".pdf";
  const targetDir = isPdf
    ? path.join(process.cwd(), "public", "documents")
    : PRO_UPLOAD_DIR;
  const publicBase = isPdf ? "/documents" : PRO_UPLOAD_PUBLIC;

  await fs.mkdir(targetDir, { recursive: true });
  const buffer = Buffer.from(await file.arrayBuffer());
  await fs.writeFile(path.join(targetDir, filename), buffer);

  return NextResponse.json({ ok: true, url: `${publicBase}/${filename}` });
}
